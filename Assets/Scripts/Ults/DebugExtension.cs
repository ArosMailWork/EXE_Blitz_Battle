...
public static class DebugExtension
{
	...
	public static void Debug2DArrow(Vector3 startPoint, Vector3 endPoint, bool reverse = true)
	{
		// Calculate the arrowhead size based on the length of the ray
		float arrowheadSize = 0.2f;

		Vector3 direction;
		if(reverse) direction = (endPoint - startPoint);
		else direction = (startPoint - endPoint);
		
		// Calculate the rotation quaternion based on the direction
		Quaternion rotation = Quaternion.LookRotation(direction);
		
		// Draw the arrowhead as a cone at the end point of the ray
		Debug.DrawRay(endPoint, direction, Color.white, 1.5f);
		Debug.DrawRay(endPoint, rotation * Quaternion.Euler(0, 150, 0) * (Vector3.back * arrowheadSize), Color.white, 1.5f);
		Debug.DrawRay(endPoint, rotation * Quaternion.Euler(0, -150, 0) * (Vector3.back * arrowheadSize), Color.white, 1.5f);
	}
	
	public static void Draw2DArrowGizmo(Vector3 startPoint, Vector3 endPoint, bool reverse = true)
	{
		// Calculate the arrowhead size based on the length of the ray
		float arrowheadSize = 0.2f;
		Vector3 direction;
		...
		Quaternion rotation = Quaternion.LookRotation(direction);
		// Draw the arrowhead as a cone at the end point of the ray
		Gizmos.DrawRay(endPoint, direction);
		Gizmos.DrawRay(endPoint, rotation * Quaternion.Euler(0, 150, 0) * (Vector3.back * arrowheadSize));
		Gizmos.DrawRay(endPoint, rotation * Quaternion.Euler(0, -150, 0) * (Vector3.back * arrowheadSize));
	}
